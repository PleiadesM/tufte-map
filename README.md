...
- **Summits.** Each hill is anchored on its most important note, drawn in red and named in small capitals. Red numerals rank the hills by height.
- **Dots.** One dot per note, all of one size (the most important a tenth larger). Its colour runs from warm grey to red with the note's importance: its links by default, weighted as you choose, with note length and recent edits as further weights.
...
Settings: excluded folders, where the map opens, how many notes to name, a text fade threshold (like the graph view's), dot size, the importance weights (inbound links, outbound links, note length, recent edits), links between summits, all links.
...
4. Summits anchored on the most important note of each hill.
...
- **0.2.0** (2026-09-23)
  - Colour a dot from warm grey to red by importance
  - Draw every dot at one size, the most important a tenth larger
  - Add a dot size slider
  - Add sliders for what importance means: inbound links, outbound links, note length, recent edits
  - Name summits and notes by importance
- **0.1.1** (2026-09-23)
